...
insert into _room_nodes select distinct e.to_room_id, 0, 0 from room_links as e where e.to_room_id is not null and e.to_room_id not in (select room_id from _room_nodes);
insert into _room_nodes select distinct e.room_id, 0, 0 from membership_events as e where e.room_id is not null and e.room_id not in (select _room_nodes.room_id from _room_nodes);
...
update _room_nodes set is_redacted = (select case when count(*) > 0 then 1 else 0 end from membership_events as e where e.room_id = _room_nodes.room_id and (e.type = 'kick' or e.type = 'ban'));

-- Find all the user nodes
create table _user_nodes (user_id text, timestamp text, is_redacted integer);
insert into _user_nodes select distinct e.sender, 0, 0 from membership_events as e;
...
insert into nodes (type, objectId, isReal, firstTimestamp, isRedacted) select 'room', room_id, 1, timestamp, is_redacted from _room_nodes;
insert into nodes (type, objectId, isReal, firstTimestamp, isRedacted) select 'user', user_id, 1, timestamp, is_redacted from _user_nodes;
...
insert into state_events (type, linkId, timestamp) select 'link_removed', links.id, (select e.timestamp + 1 from timeline_events as e where e.linkId = links.id) from links where links.isVisible = 0;
